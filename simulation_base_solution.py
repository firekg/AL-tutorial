...
def bayesian_active_sensor(prior, lik):
    # BAS(x') = [sum_h P(h)H(y|x',h)] - H(y|x')
    n_hyp, n_features, n_labels = lik.shape
    pred = predictive(prior, lik)
    bas = np.zeros(n_features)
    for j in range(n_features):
        weighted_ent_lik = 0
        for i in range(n_hyp):
            weighted_ent_lik += prior[i]*entropy(lik[i,j,:][:])
        bas[j] = weighted_ent_lik - entropy(pred[j,:][:])
    return bas
...
print(eig)
bas = bayesian_active_sensor(prior, lik)
print(bas)
# Expected output:
...
for step in range(n_step):
    ...
    post = posterior([x_select], [y], prior, lik)
